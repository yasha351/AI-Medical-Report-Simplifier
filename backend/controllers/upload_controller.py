...
from services.parser.lab_pipeline import process_lab_report
...
async def handle_upload(file: UploadFile, upload_type: str = "lab"):
    ...
    try:
        if upload_type == "prescription":
            result = process_prescription(str(destination_path))
        else:
            result = process_lab_report(str(destination_path))
